// =============================================
// JS Legal Force - Kennisquiz Geweldsaanwending
// =============================================
...
document.getElementById('btn-naar-start').addEventListener('click', () => {
  ...
  document.getElementById('start-welkom').innerHTML = `Welkom, ${voornaam}!<br><span style="font-size:1.3rem;font-weight:600">Geweldsaanwending Kennisquiz</span>`;
  toonScherm('start');
});
...
function toonVraag() {
  ...
  document.getElementById('btn-volgende').textContent =
    huidigVraagIndex < ALLE_VRAGEN.length - 1 ? 'Volgende vraag ->' : 'Bekijk resultaat ->';
...
function toonEinde() {
  ...
  document.getElementById('eind-beoordeling-tekst').textContent = geslaagd
    ? 'Gefeliciteerd! Je hebt de kennisquiz Geweldsaanwending met goed gevolg afgerond.'
  ...
  btnCert.textContent = geslaagd
    ? 'Download bewijs van deelname (PDF)'
    : 'Download resultatenoverzicht (PDF)';
...
document.getElementById('btn-certificaat').addEventListener('click', () => {
  ...
  doc.text("Kennisquiz Geweldsaanwending - voor BOA's", W / 2, 23, { align: 'center' });
  ...
  doc.text("Kennisquiz Geweldsaanwending voor BOA's", W / 2, 72, { align: 'center' });
  ...
  const naamTekst = deelnemerNaam.charAt(0).toUpperCase() + deelnemerNaam.slice(1) + ' ' +
    deelnemerAchternaam.charAt(0).toUpperCase() + deelnemerAchternaam.slice(1);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(24);
  doc.setTextColor(26, 42, 94);
  ...
  doc.setTextColor(26, 42, 94);
  if (geslaagd) {
    doc.text('Heeft de kennisquiz Geweldsaanwending van JS Legal Force met goed gevolg afgerond.', W / 2, 126, { align: 'center' });
  } else {
    doc.text('Heeft deelgenomen aan de kennisquiz Geweldsaanwending van JS Legal Force.', W / 2, 123, { align: 'center' });
  ...
  doc.setFontSize(8);
  doc.setTextColor(138, 154, 181);
  doc.text("(c) JS Legal Force - Kennisquiz Geweldsaanwending voor BOA's", W / 2, H - 10, { align: 'center' });

  const prefix = geslaagd ? 'Bewijs_Deelname' : 'Resultaten';
  doc.save(`${prefix}_Geweldsaanwending_${deelnemerNaam}_${deelnemerAchternaam}_${nu.getFullYear()}.pdf`);
});
...
function genereerNummer() {
  ...
  return `JSLF-GA-${jaar}${maand}${dag}-${rand}`;
}
